...
def rps(user):
    ...
    if user == computer:
        var.set("It's a draw.\nNo Points")
    elif (user == 1 and computer == 3) or (user == 2 and computer == 1) or (user == 3 and computer == 2) or (
            user == 4 and computer == 3) or (user == 5 and computer == 4):
        var.set("You win!")
        win += 1
        wins.set(win)
    else:
